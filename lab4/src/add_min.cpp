...
void propagate(std::vector<long long> &tree, size_t x) {
    tree[2 * x + 1] += tree[x];
    tree[2 * x + 2 ] += tree[x];
    tree[x] = 0;
}
void add(std::vector<long long > &tree, std::vector<long long > &tree_min, size_t l, size_t r, long long v, size_t x, size_t lx, size_t rx) {
    if (l >=  rx || lx >= r) {
        return;
    }
    if (lx >= l && rx <= r) {
    ...
    //propagate(tree, x);
    size_t m = (lx + rx) / 2;
    add(tree, tree_min, l, r, v, 2 * x + 1, lx, m);
    add(tree, tree_min, l, r, v, 2 * x + 2, m, rx);
    tree_min[x] = std::min(tree_min[2 * x + 1], tree_min[2 * x + 2]) + tree[x];
}
...
long long min(std::vector<long long > &tree, std::vector<long long > &tree_min, size_t l, size_t r, size_t x, size_t lx, size_t rx) {
    ...
    return std::min(min1, min2) + tree[x];
}
...
int main() {
    ...
    for (size_t i = 0; i < num_op; i++) {
        ...
        if (op == 1) {
            long long v;
            std::cin >> v;
            add(tree, tree_min, l, r, v, 0, 0, len);
